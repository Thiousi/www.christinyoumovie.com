...
<?php else: ?>
  <section id="banner" class="video-bg">
    <div id="js-parallax-window" class="parallax-window">
      ...
      <?php endif ?>
      <div data-parallax="background" id="js-parallax-background" class="<?php if($page->isHomePage() || $page->fullscreen()->isTrue() ) { echo ''; } else { echo 'fixed-height ';} ?>parallax-background" style="background-position: center center; background-size: cover; background-image: url('<?php if ($page->backgroundimage()->isNotEmpty()): ?><?php echo $page->backgroundimage()->toFile()->url() ?><?php else: ?><?php echo $site->image('background.jpg')->url() ?><?php endif; ?>');">
        <video muted autoplay loop poster="<?php echo $page->backgroundimage()->toFile()->url() ?>" id="bgvideo">
